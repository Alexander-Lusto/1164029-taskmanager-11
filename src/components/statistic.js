...
const getTasksByDateRange = (tasks, dayFrom, dayTo) => {
  return tasks.filter((task) => {
    const dueDate = task.dueDate;

    return dueDate >= dayFrom && dueDate <= dayTo;
  });
};

const createPlaceholder = (dayFrom, dayTo) => {
  const format = (date) => {
    return moment(date).format(`DD MMM`);
  };

  return `${format(dayFrom)} - ${format(dayTo)}`;
};
